...
import pandas as pd
import pytest
from configure_dms_viz import format_mutation_data, format_sitemap_data, join_additional_data
...
def test_format_mutation_data(dummy_data):
    ...
    # Call the function
    formatted_data = format_mutation_data(mut_metric_df, metric_col, condition_col, alphabet)
...
def test_format_sitemap_data(dummy_data):
    ...
    formatted_df = format_sitemap_data(sitemap_df, mut_metric_df, included_chains)
    ...
    assert set(mut_metric_df.reference_site).issubset(set(formatted_df.reference_site))
...
def test_join_additional_data(dummy_data):
    ...
    # Call the function
    joined_df = join_additional_data(mut_metric_df, join_data)
    ...
    assert 'additional_col' in joined_df.columns, "Expected column 'additional_info' not found in the joined dataframe."
    
    # Assert no duplicate measurements per mutation
    assert not joined_df.duplicated(subset=["reference_site", "wildtype", "mutant"]).any(), "Duplicate measurements found after join."
